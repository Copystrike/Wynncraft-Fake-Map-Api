...
public abstract class PlayerList {
   ...
   public void initializeConnectionToPlayer(NetworkManager netManager, ServerPlayerEntity playerIn) {
      ...
      ServerWorld serverworld = this.server.func_71218_a(playerIn.dimension);
   ...
   public CompoundNBT readPlayerDataFromFile(ServerPlayerEntity playerIn) {
      CompoundNBT compoundnbt = this.server.func_71218_a(DimensionType.OVERWORLD).getWorldInfo().getPlayerNBTTagCompound();
   ...
   public ServerPlayerEntity createPlayerForUser(GameProfile profile) {
      ...
      return new ServerPlayerEntity(this.server, this.server.func_71218_a(DimensionType.OVERWORLD), profile, playerinteractionmanager);
   }
   ...
   public boolean canSendCommands(GameProfile profile) {
      return this.ops.hasEntry(profile) || this.server.func_213199_b(profile) && this.server.func_71218_a(DimensionType.OVERWORLD).getWorldInfo().areCommandsAllowed() || this.commandsAllowedForAll;
   }
   ...
   public void sendToAllNearExcept(@Nullable PlayerEntity except, double x, double y, double z, double radius, DimensionType dimension, IPacket<?> packetIn) {
      for(int i = 0; i < this.players.size(); ++i) {
         ...
         if (serverplayerentity != except && serverplayerentity.dimension == dimension) {
            double d0 = x - serverplayerentity.func_226277_ct_();
            double d1 = y - serverplayerentity.func_226278_cu_();
            double d2 = z - serverplayerentity.func_226281_cx_();
