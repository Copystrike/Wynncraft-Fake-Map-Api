...
public class BeeEntity extends AnimalEntity implements IFlyingAnimal {
   ...
   protected PathNavigator createNavigator(World worldIn) {
      FlyingPathNavigator flyingpathnavigator = new FlyingPathNavigator(this, worldIn) {
         public boolean canEntityStandOnPos(BlockPos pos) {
            return !this.world.getBlockState(pos.func_177977_b()).isAir();
         }
   ...
   class FindPollinationTargetGoal extends BeeEntity.PassiveGoal {
      ...
      public void tick() {
         if (BeeEntity.this.rand.nextInt(30) == 0) {
            for(int i = 1; i <= 2; ++i) {
               BlockPos blockpos = (new BlockPos(BeeEntity.this)).func_177979_c(i);
            ...
            for(int j = 0; (double)j < p_226500_2_; ++j) {
               for(int k = 0; k <= j; k = k > 0 ? -k : 1 - k) {
                  for(int l = k < j && k > -j ? j : 0; l <= j; l = l > 0 ? -l : 1 - l) {
                     blockpos$mutable.func_189533_g(blockpos).func_196234_d(k, i - 1, l);
   ...
   class WanderGoal extends Goal {
      ...
      public void startExecuting() {
         ...
         if (vec3d != null) {
            BeeEntity.this.navigator.setPath(BeeEntity.this.navigator.func_179680_a(new BlockPos(vec3d), 1), 1.0D);
         }
