        ...
        static ArgumentBuilder<CommandSource, ?> register()
        {
            return Commands.literal("list")
                ...
                .then(Commands.argument("filter", StringArgumentType.string())
                    ...
                    .executes(ctx -> execute(ctx.getSource(), StringArgumentType.getString(ctx, "filter"), ctx.getSource().func_197023_e().dimension.getType()))
                )
                .executes(ctx -> execute(ctx.getSource(), "*", ctx.getSource().func_197023_e().dimension.getType()));
        }
